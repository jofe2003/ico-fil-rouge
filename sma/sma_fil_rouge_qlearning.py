...
QL_OUTER_CYCLES = 10
QL_ITER_PER_CYCLE = 120
QL_ALPHA = 0.10
QL_GAMMA = 0.90
QL_EPSILON_START = 0.35
QL_EPSILON_DECAY = 0.96
QL_EPSILON_MIN = 0.05
QL_FRIEND_RATE = 0.25
...
def greedy_insert_customers(routes, customers_to_insert, instance):
    cand = [r[:] for r in routes]
...
def ql_intra_route_swap(routes, instance):
    cand = [r[:] for r in routes]
...
def ql_inter_route_swap(routes, instance):
    cand = [r[:] for r in routes]
    candidates = [k for k, r in enumerate(cand) if r]
    if len(candidates) < 2:
        return cand
...
def ql_intra_route_shift(routes, instance):
    cand = [r[:] for r in routes]
    candidates = [k for k, r in enumerate(cand) if len(r) >= 2]
    if not candidates:
        return cand
...
def ql_inter_route_shift(routes, instance):
    cand = [r[:] for r in routes]
    sources = [k for k, r in enumerate(cand) if r]
    if not sources or len(cand) < 2:
        return cand
    ks = random.choice(sources)
    possible_dest = [k for k in range(len(cand)) if k != ks]
    kd = random.choice(possible_dest)
...
def ql_two_intra_route_swap(routes, instance):
    cand = [r[:] for r in routes]
    candidates = [k for k, r in enumerate(cand) if len(r) >= 4]
    if not candidates:
        return cand
    ...
    for a in pairs:
        for b in pairs:
            if a[1] < b[0] or b[1] < a[0]:
                feasible.append((a[0], b[0]))
    if not feasible:
        return cand
    i, j = random.choice(feasible)
    cand[k][i], cand[k][j] = cand[k][j], cand[k][i]
    cand[k][i + 1], cand[k][j + 1] = cand[k][j + 1], cand[k][i + 1]
    return cand


def ql_two_intra_route_shift(routes, instance):
    cand = [r[:] for r in routes]
    candidates = [k for k, r in enumerate(cand) if len(r) >= 4]
    if not candidates:
        return cand
    ...
    pair = [cand[k].pop(i), cand[k].pop(i)]
    j = random.randrange(len(cand[k]) + 1)
    cand[k][j:j] = pair
    return cand


def ql_eliminate_smallest_route(routes, instance):
    cand = [r[:] for r in routes]
    non_empty = [(len(r), k) for k, r in enumerate(cand) if r]
    if len(non_empty) < 2:
        return cand
    _, k_remove = min(non_empty)
    removed = cand[k_remove][:]
    cand[k_remove] = []
    return greedy_insert_customers(cand, removed, instance)


def ql_eliminate_random_route(routes, instance):
    cand = [r[:] for r in routes]
    non_empty = [k for k, r in enumerate(cand) if r]
    if len(non_empty) < 2:
        return cand
    k_remove = random.choice(non_empty)
    removed = cand[k_remove][:]
    cand[k_remove] = []
    return greedy_insert_customers(cand, removed, instance)
...
QL_ACTIONS = [
    ('intra_route_swap', ql_intra_route_swap),
    ('inter_route_swap', ql_inter_route_swap),
    ('intra_route_shift', ql_intra_route_shift),
    ('inter_route_shift', ql_inter_route_shift),
    ('two_intra_route_swap', ql_two_intra_route_swap),
    ('two_intra_route_shift', ql_two_intra_route_shift),
    ('eliminate_smallest_route', ql_eliminate_smallest_route),
    ('eliminate_random_route', ql_eliminate_random_route),
]


def ql_state(previous_delta, stagnation):
    if previous_delta > 100.0:
        return 0
    if previous_delta > 1e-9:
        return 1
    if stagnation < 10:
        return 2
    return 3
...
def ql_choose_action(q_table, state, epsilon):
...
def ql_apply_action(routes, instance, action_idx):
    return QL_ACTIONS[action_idx][1](routes, instance)


def q_learning_agent(instance, pool, seed=45):
    random.seed(seed)
    np.random.seed(seed)
    result = AgentResult('q_learning')

    current = excel_order_initial_solution(instance)
    current.source = 'q_learning'
    best = current.clone()
    pool.add(best)

    q_table = np.zeros((4, len(QL_ACTIONS)), dtype=float)
    epsilon = QL_EPSILON_START
    previous_delta = 0.0
    stagnation = 0
    q_history = []

    for cycle in range(QL_OUTER_CYCLES):
        for _ in range(QL_ITER_PER_CYCLE):
            state = ql_state(previous_delta, stagnation)
            action_idx = ql_choose_action(q_table, state, epsilon)
            candidate_routes = ql_apply_action(current.routes, instance, action_idx)
            candidate = evaluate(instance, candidate_routes, source='q_learning')
            reward = current.penalized - candidate.penalized
            next_state = ql_state(max(0.0, reward), 0 if reward > 1e-9 else stagnation + 1)

            q_table[state, action_idx] = (
                (1.0 - QL_ALPHA) * q_table[state, action_idx]
                + QL_ALPHA * (reward + QL_GAMMA * np.max(q_table[next_state]))
            )

            if reward > 0 or random.random() < 0.05:
                current = candidate
                previous_delta = max(0.0, reward)
                stagnation = 0 if reward > 0 else stagnation + 1
            else:
                previous_delta = 0.0
                stagnation += 1
        ...
        if random.random() < QL_FRIEND_RATE:
            friend = pool.sample_friend(exclude_source='q_learning')
            if friend is not None:
                result.friend_imports += 1
                candidate = intensify_from_friend(friend, instance)
                candidate.source = 'q_learning'
        ...
        epsilon = max(QL_EPSILON_MIN, epsilon * QL_EPSILON_DECAY)

    best.source = 'q_learning'
    result.best = best
    result.q_table = q_table
    result.q_history = q_history
    result.action_names = [name for name, _ in QL_ACTIONS]
    return result


def save_q_learning_outputs(result, output_dir):
    if not hasattr(result, 'q_table'):
        return
    q_df = pd.DataFrame(result.q_table, columns=result.action_names)
    q_df.insert(0, 'state', ['S1_big_improvement', 'S2_small_improvement', 'S3_no_improvement', 'S4_stagnation'])
    q_df.to_csv(Path(output_dir) / 'q_learning_table.csv', index=False)

    rows = []
    for t, table in enumerate(result.q_history, start=1):
        for s in range(table.shape[0]):
            for a, name in enumerate(result.action_names):
                rows.append({'cycle': t, 'state': s + 1, 'action': name, 'q_value': table[s, a]})
    pd.DataFrame(rows).to_csv(Path(output_dir) / 'q_learning_history.csv', index=False)
...
def run_sma_amis(instance, output_dir, seed=42):
    ...
    ql = q_learning_agent(instance, pool, seed=seed + 3)
    ...
    save_q_learning_outputs(ql, output_dir)
