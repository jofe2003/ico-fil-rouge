...
class EnemyPool:
    ...
    def sample_enemy(self, exclude_source=None):
...
def diversify_against_enemy(base, enemy, instance, pool, attempts=12):
    base_dist = pool.lambda_distance(base, enemy)
    best = None
    best_rank = float('inf')
    ...
    for _ in range(attempts):
        ...
        candidate = evaluate(instance, routes, source=base.source)
        dist = pool.lambda_distance(candidate, enemy)
        gain_dist = max(0.0, dist - base_dist)
        rank = candidate.penalized - 25.0 * gain_dist

        if dist > base_dist and rank + 1e-9 < best_rank:
            best = candidate
            best_rank = rank

    if best is None:
        routes = mutate_routes(base.routes, instance)
        best = evaluate(instance, routes, source=base.source)
    ...
    for _ in range(GA_OUTER_CYCLES):
        ...
        while len(offspring) < GA_OFFSPRING_PER_CYCLE:
            ...
            if random.random() < GA_ENEMY_RATE:
                enemy = pool.sample_enemy(exclude_source='genetic')
                if enemy is not None:
                    result.enemy_imports += 1
                    enemy_child = diversify_against_enemy(child, enemy, instance, pool)
                    if enemy_child.penalized + 1e-9 < child.penalized:
...
def simulated_annealing_agent(instance, pool, seed=43):
    ...
    for _ in range(SA_OUTER_CYCLES):
        ...
        if random.random() < SA_ENEMY_RATE:
            enemy = pool.sample_enemy(exclude_source='annealing')
            if enemy is not None:
                result.enemy_imports += 1
                candidate = diversify_against_enemy(current, enemy, instance, pool)
...
def tabu_agent(instance, pool, seed=44):
    ...
    for _ in range(TABU_OUTER_CYCLES):
        ...
        if random.random() < TABU_ENEMY_RATE:
            enemy = pool.sample_enemy(exclude_source='tabu')
            if enemy is not None:
                result.enemy_imports += 1
                candidate = diversify_against_enemy(current, enemy, instance, pool)
...
def plot_enemy_activity(results, output_dir):
    names = [r.name for r in results]
    imports = [r.enemy_imports for r in results]
    gains = [r.enemy_improvements for r in results]
    x = np.arange(len(results))

    plt.figure(figsize=(10, 6))
    plt.bar(x - 0.18, imports, 0.36, label='Enemy uses')
    plt.bar(x + 0.18, gains, 0.36, label='Useful gains')
    plt.xticks(x, names)
    plt.ylabel('Count')
    plt.title('Enemy competition activity')
    plt.legend()
    plt.grid(True, axis='y', alpha=0.3)
    plt.tight_layout()
...
def save_summary(instance, results, best, output_dir):
    ...
    for res in results:
        rows.append({
            ...
            'enemy_imports': res.enemy_imports,
...
def print_report(instance, results, best, elapsed):
    print('=' * 72)
    print('SMA VRP - Concurrence Ennemis')
    print('=' * 72)
    ...
    print('-' * 72)
    for res in results:
        print(f"{res.name:<12} penalized={res.best.penalized:>12.2f} | f(x)={res.best.objective:>12.2f} | penalties={res.best.penalties:>10.2f} | imports={res.enemy_imports:>3d} | gains={res.enemy_improvements:>3d}")
    print('-' * 72)
...
if __name__ == '__main__':
    ...
    for route_id in ALL_ROUTES:
        ...
        rid = choose_route_id(customers, route_id=route_id)
        output_dir = root / 'resultats_images' / 'enemies' / f'sma_enemie_route_{route_id}'
